...
export function appleAuthMiddleware(req, res, next) {
  ...
  console.log('[APPLE AUTH] Header recibido:', authHeader?.substring(0, 50) + '...');
  console.log('[APPLE AUTH] Token esperado:', `ApplePass ${expectedToken?.substring(0, 10)}...`);
  
  if (!authHeader || authHeader !== `ApplePass ${expectedToken}`) {
    console.warn('[APPLE AUTH] ❌ Token inválido o faltante');
    console.warn('[APPLE AUTH] Comparación completa:');
    console.warn('[APPLE AUTH]   Recibido:', authHeader);
    console.warn('[APPLE AUTH]   Esperado:', `ApplePass ${expectedToken}`);
