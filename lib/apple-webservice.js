...
export function appleAuthMiddleware(req, res, next) {
  ...
  if (!authHeader || authHeader !== `ApplePass ${expectedToken}`) {
    console.warn('[APPLE AUTH] ⚠️ Token inválido o faltante');
    console.warn('[APPLE AUTH] Esperado:', `ApplePass ${expectedToken?.substring(0, 10)}...`);
    console.warn('[APPLE AUTH] Recibido:', authHeader?.substring(0, 30) + '...');
