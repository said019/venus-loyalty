...
/* =========================================================
   ...
   ========================================================= */
/* =========================================================
   ENVIAR NOTIFICACIÓN PUSH APNs (ALERTA VISIBLE EN PANTALLA BLOQUEO)
   ========================================================= */

async function sendAPNsPushNotification(pushToken, title = "Venus Cosmetología", body = "Tienes sellos acumulados") {
  ...
  try {
    ...
    // ✅ PAYLOAD PARA ALERTA VISIBLE EN PANTALLA BLOQUEO
    const apnsPayload = {
      "aps": {
        "alert": {
          "title": title,
    ...
    console.log(`[APNs/ALERT] 📤 Enviando ALERTA VISIBLE a: ${pushToken.substring(0, 10)}...`);
    ...
    return new Promise((resolve, reject) => {
      ...
      const req = client.request({
        ...
        // ✅ CONFIGURACIÓN PARA ALERTAS VISIBLES
        'authorization': `bearer ${token}`,
        'apns-topic': apnsTopic,
        'apns-push-type': 'alert',        // ⭐ CRÍTICO: 'alert' en lugar de 'background'
        'apns-priority': '10',            // ⭐ CRÍTICO: Prioridad alta para alertas
      ...
      req.on('response', (headers) => {
        ...
        req.on('end', () => {
          ...
          if (statusCode === 200) {
            console.log('[APNs/ALERT] ✅ Alerta VISIBLE enviada exitosamente');
            resolve(true);
          } else {
            let errorBody = data;
            try {
...
/* =========================================================
   NOTIFICAR A TODOS LOS DISPOSITIVOS DE UNA TARJETA (AHORA VISIBLE)
   ========================================================= */
...
export async function notifyCardUpdate(cardId, title = "Venus Cosmetología", message = "Tu tarjeta ha sido actualizada") {
  try {
    ...
    console.log(`[APPLE] 📢 Enviando ALERTAS VISIBLES a ${devices.length} dispositivo(s) para ${cardId}`);
    ...
    for (const device of devices) {
      try {
        // ✅ Ahora usa la función modificada con título y mensaje
        await sendAPNsPushNotification(device.push_token, title, message);
        sent++;
      } catch (error) {
    ...
    console.log(`[APPLE] ✅ Alertas visibles enviadas: ${sent} de ${devices.length}`);
