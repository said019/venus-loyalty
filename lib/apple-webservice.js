...
import fs from 'fs';
import http2 from 'node:http2'; // <--- AGREGA ESTO
...
function generateAPNsToken() {
 ...
 /* =========================================================
   ENVIAR NOTIFICACIÓN PUSH APNs (VERSIÓN HTTP/2)
   ========================================================= */

function sendAPNsPushNotification(pushToken) {
  return new Promise((resolve, reject) => {
    try {
      const token = generateAPNsToken();
      const topic = process.env.APPLE_PASS_TYPE_ID;
      
      // Conectar usando HTTP/2
      ...
      const req = client.request({
        ...
        'apns-topic': topic,
        'apns-push-type': 'background', // Importante para Wallet
        'apns-priority': '5',           // Prioridad baja (background)
        'apns-expiration': '0'          // Intentar entregar inmediatamente
      });
      ...
      req.on('response', (headers) => {
        ...
        if (status === 200) {
          console.log('[APNs] ✅ Push enviado correctamente (HTTP/2)');
        } else {
      ...
      req.on('end', () => {
        client.close(); // Cerrar sesión al terminar
        if (data) {
            // Si hay error, Apple suele mandar un JSON
            try { console.error('[APNs] Response:', JSON.parse(data)); } catch(e){}
        }
        resolve(true);
      });
      ...
      // Enviar payload vacío (Wallet solo necesita "despertar")
      req.write(JSON.stringify({}));
      req.end();

    } catch (error) {
      console.error('[APNs] Excepción fatal:', error);
      reject(error);
    }
  });
}
...
/* =========================================================
   ⭐ HELPER DE AUTH INLINE (CORREGIDO)
   ========================================================= */

function checkAuth(req, res) {
  // Express normaliza la cabecera a minúsculas: 'authorization'
  const authHeader = req.headers.authorization;
  const expectedToken = APPLE_AUTH_TOKEN;

  console.log('[APPLE AUTH] 🔐 Verificando autenticación...');
  // Logueamos solo una parte del token para seguridad
...
export async function getUpdatablePassesHandler(req, res) {
  ...
  // ⭐ AUTH INLINE - Aquí se usa la función checkAuth() corregida
  if (!checkAuth(req, res)) return;
  ...
  try {
    ...
    if (rows.length === 0) {
      ...
      return res.status(204).send(); // 204 No Content es la respuesta correcta de Apple
    }
    ...
    if (passesUpdatedSince) {
      ...
      for (const serial of serialNumbers) {
        ...
        // Si no hay registro de actualización o si la última actualización es posterior a 'since'
...
export async function unregisterDeviceHandler(req, res) {
  ...
  // ⭐️ MODIFICACIÓN CLAVE: NO USAR checkAuth ESTRICTO
  // Los pases viejos (zombies) tienen tokens incorrectos y necesitan ser desregistrados.
  // Si respondemos 401, el iPhone se queda en un bucle de reintentos.
  const authHeader = req.headers.authorization;
  const expectedToken = process.env.APPLE_AUTH_TOKEN; // Asumimos que esta variable es el token correcto
  ...
  if (authHeader && authHeader.startsWith('ApplePass ')) {
    ...
    if (receivedToken !== expectedToken) {
      ...
      // Este caso solo ocurre si la conexión es muy vieja o corrupta
      console.warn('[APPLE AUTH] ⚠️ Falta token. Permitiendo desregistro de "Pase Zombie" (para evitar 401 loop).');
  }
  
  // Nota: Si la ruta tiene un middleware de Express que llama a checkAuth, debes quitarlo.
  // Aquí estamos asumiendo que el checkAuth estaba en esta función.
  ...
  try {
    ...
    // IMPORTANTE: Responder 200 OK para confirmar al iPhone que lo borró.
    res.status(200).send();
    
  } catch (error) {
    console.error('[APPLE] ❌ Error desregistrando dispositivo:', error);
    // ⭐️ CAMBIO CRUCIAL: Si hay un error en la DB (ej. el registro no existía),
    // debemos responder 200 OK para que el iPhone deje de reintentar.
    // Para el iPhone, la acción de "desregistro" debe considerarse exitosa.
    res.status(200).send(); 
  }
}
