...
function getAPNsConfig() {
  ...
  if (process.env.APPLE_APNS_KEY_BASE64) {
     keyContent = Buffer.from(process.env.APPLE_APNS_KEY_BASE64, 'base64').toString('utf8');
  } else if (process.env.APPLE_APNS_KEY_PATH) {
     if (fs.existsSync(process.env.APPLE_APNS_KEY_PATH)) {
        keyContent = fs.readFileSync(process.env.APPLE_APNS_KEY_PATH, 'utf8');
     }
  }

  if (!keyId || !teamId || !keyContent) {
    throw new Error('Faltan credenciales de APNs: APPLE_KEY_ID, APPLE_TEAM_ID, y (APPLE_APNS_KEY_BASE64 o APPLE_APNS_KEY_PATH)');
  }
