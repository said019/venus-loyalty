...
async getUpdatablePassesHandler(req, res) {
  try {
    ...
    if (devicesSnapshot.empty) {
      return res.json({ 
        serialNumbers: [],
        lastUpdated: new Date().toISOString()  // ⭐ AGREGADO
      });
    }

    const serialNumbers = devicesSnapshot.docs.map((doc) => doc.data().serial_number);
    
    console.log(`[APPLE] 📋 ${serialNumbers.length} pase(s) actualizables`);

    // ⭐ CRÍTICO: Siempre incluir lastUpdated
    ...
    console.log(`[APPLE] ✅ Respuesta con lastUpdated: ${lastUpdated}`);
  } catch (error) {
    console.error("[APPLE HANDLER] Error en getUpdatablePassesHandler:", error);
...
export async function getLatestPassHandler(req, res) {
  ...
  // ⭐ AUTH INLINE - REQUERIDO: Esta ruta SÍ requiere autenticación.
  if (!checkAuth(req, res)) return;
