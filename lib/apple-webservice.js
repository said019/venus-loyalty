...
export async function getLatestPassHandler(req, res) {
  ...
  try {
    ...
    // Obtener tarjeta desde Firestore (INCLUYENDO EL MENSAJE)
    const card = await fsGetCard(serial);
    if (!card) {
    ...
    const passData = {
      ...
      max: card.max || 8,
      latestMessage: card.latestMessage || null // ⭐ Esto se usará en buildApplePassBuffer
    };

    console.log('[APPLE] 🔨 Generando pase con datos:', passData);
