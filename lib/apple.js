...
export async function buildApplePassBuffer({ cardId, name, stamps, max }) {
  try {
    ...
    // 7) Verificación simplificada - solo logs de confirmación
    console.log(`[Apple Wallet] Verificación de imágenes completada:`);
    console.log(`[Apple Wallet] ✅ Logo incluido`);
    console.log(`[Apple Wallet] ✅ Icon incluido`); 
    console.log(`[Apple Wallet] ✅ Strip incluido`);
