...
function buildTempModelDir({
  ...
  const passJson = {
    ...
    generic: {
      ...
      // Sin campos auxiliares (sin ID en el frente)
      auxiliaryFields: [],
    ...
    backgroundColor: "rgb(154, 159, 130)", // gris-verde
...
export async function buildApplePassBuffer({ cardId, name, stamps, max }) {
  try {
    ...
    // 2) STRIP - Imagen con los sellos dinámicos
    ...
    if (!stripFound) {
      console.warn(`[Apple Wallet] ⚠️  No se encontró strip para ${safeStamps} sellos`);
