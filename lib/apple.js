...
import { FUENTES_APPLE, rutaDe } from "./apple-credenciales.js";
...
// Normaliza saltos de línea escapados que vienen en env vars (\n, \r\n, \\n).
...
function extractFirstCert(pem) {
  const m = pem.match(
    /-----BEGIN CERTIFICATE-----[\s\S]+?-----END CERTIFICATE-----/
  );
  return m ? m[0].trim() : pem;
}

// Carga un PEM aceptando: (1) contenido inline desde env var —con saltos de
// línea escapados o en base64—, o (2) una ruta a archivo (legacy). Así los
// passes firman igual en Render (rutas a archivo) y en Railway (env vars inline).
function readPemFlexible(inlineValue, filePath, label, certOnly) {
  // 1) Contenido inline desde env var.
  if (inlineValue && String(inlineValue).trim()) {
    let pem = unescapePemNewlines(inlineValue);
    // Si no trae headers PEM pero parece base64, decodificar.
    if (!/-----BEGIN/.test(pem) && /^[A-Za-z0-9+/=\s]+$/.test(pem)) {
      try {
        const decoded = Buffer.from(String(inlineValue).trim(), "base64").toString("utf8");
        if (/-----BEGIN/.test(decoded)) pem = decoded.trim();
      } catch { /* no era base64 válido */ }
    }
    if (/-----BEGIN/.test(pem)) return certOnly ? extractFirstCert(pem) : pem;
  }
  // 2) Ruta a archivo (comportamiento legacy / Render).
  if (filePath && fs.existsSync(filePath)) {
    const raw = fs.readFileSync(filePath, "utf8").trim();
    return certOnly ? extractFirstCert(raw) : raw;
  }
  throw new Error(`[Apple Wallet] No se pudo cargar ${label} (ni env var inline ni archivo)`);
}
...
function buildTempModelDir({
  ...
  // Configuración según tipo de tarjeta
  const isMassage = cardType === 'massage';
  const isAnnual = cardType === 'annual';
  const isGold = cardType === 'gold';

  const passConfig = isMassage ? {
    description: "Membresía Masajes Venus",
    logoText: "Venus Masajes",
    stampsLabel: "SESIONES",
    stampsValue: `${stamps} de ${max} sesiones`,
    programLabel: "MEMBRESÍA",
    programValue: "Masajes Venus",
    termsText: "Completa tus sesiones de masaje.\n\nVálido en Venus Cosmetología.",
    locationText: "¡Estás cerca de Venus! Muestra tu membresía de masajes",
  } : isAnnual ? {
    description: "Membresía Anual Venus",
    logoText: "Venus Cosmetología",
    stampsLabel: "SESIONES",
    stampsValue: `${stamps} de ${max} sesiones`,
    programLabel: "PROGRAMA",
    programValue: "Membresía Anual",
    termsText: "Membresía anual con sesiones incluidas.\n\nVálido en Venus Cosmetología.",
    locationText: "¡Estás cerca de Venus! Muestra tu membresía",
  } : isGold ? {
    description: "Tarjeta Gold Venus",
    logoText: "Venus Cosmetología",
    stampsLabel: "SELLOS",
    stampsValue: `${stamps} de ${max}`,
    programLabel: "PROGRAMA",
    programValue: "Venus Gold",
    termsText: "Tarjeta Gold VIP.\n\nVálido en Venus Cosmetología.",
    locationText: "¡Estás cerca de Venus! Muestra tu tarjeta Gold",
  } : {
    ...
    stampsValue: `${stamps} de ${max}`,
    programLabel: "PROGRAMA",
    programValue: "Lealtad Venus",
    termsText: "Completa tus sellos y canjea un facial gratis.\n\nVálido en Venus Cosmetología.",
    locationText: "¡Estás cerca de Venus! Muestra tu tarjeta de lealtad",
  };
  ...
  const auxiliaryFields = [
    {
      key: "program",
      label: passConfig.programLabel,
      value: passConfig.programValue,
    },
  ];
  ...
  const passJson = {
    ...
    storeCard: {
      ...
      secondaryFields: [
        ...
        {
          key: "balance",
          label: passConfig.stampsLabel,
          value: passConfig.stampsValue,
        },
      ],
    ...
    locations: [
      {
        latitude: parseFloat(process.env.BUSINESS_LATITUDE || "20.3787658"),
        longitude: parseFloat(process.env.BUSINESS_LONGITUDE || "-99.9629932"),
        relevantText: passConfig.locationText,
      }
    ],
...
export async function buildApplePassBuffer({
  ...
  try {
    ...
    const bgColorMap = {
      ...
      massage: 'rgb(196, 147, 110)', // Venus terracota — sin esta entrada, el pase
      // de masajes se repintaba verde-lealtad en cada actualización (la ruta de
      // update de apple-webservice.js no pasa cardColor y caía al fallback)
    };
    ...
    // Inline (Railway: APPLE_SIGNER_*_PEM) con fallback a ruta de archivo
    // (Render: APPLE_PASS_CERT / APPLE_PASS_KEY / APPLE_WWDR) y al final a los
    // archivos versionados en el repo. Las fuentes viven en
    // lib/apple-credenciales.js, que es lo mismo que consulta el botón de la
    // tarjeta pública: si cambian aquí, cambian allá.
    const { signerCert, signerKey, wwdr } = FUENTES_APPLE;
    const signerCertPem = readPemFlexible(
      process.env[signerCert.inline],
      rutaDe(signerCert),
      "APPLE_SIGNER_CERT_PEM / APPLE_PASS_CERT (pass.pem)",
      true
    );
    const signerKeyPem = readPemFlexible(
      process.env[signerKey.inline],
      rutaDe(signerKey),
      "APPLE_SIGNER_KEY_PEM / APPLE_PASS_KEY (pass.key)",
      false
    );
    const wwdrPem = readPemFlexible(
      process.env[wwdr.inline],
      rutaDe(wwdr),
      "APPLE_WWDR_CERT_PEM / APPLE_WWDR (wwdr_rsa.pem · G4/RSA)",
      true
    );
    ...
    // STRIP con los sellos — usar imágenes de masaje o lealtad según cardType
