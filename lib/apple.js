...
function buildTempModelDir({
  ...
  // ⭐ AGREGAR CAMPO DE NOTIFICACIÓN SI HAY MENSAJE
  if (latestMessage) {
    auxiliaryFields.push({
      key: "latestNotification",
      label: "📢 Último Aviso",
      value: latestMessage,
      changeMessage: "Nuevo mensaje: %@" // ⭐ ESTO GENERA LA NOTIFICACIÓN VISIBLE
    });
    console.log(`[APPLE MODEL] 🔔 Incluyendo mensaje en modelo: "${latestMessage}"`);
  }
  ...
  const passJson = {
    ...
    storeCard: {
      ...
      auxiliaryFields: auxiliaryFields, // ⭐ USAR ARRAY DINÁMICO
    ...
    relevantDate: relevantDate || new Date(Date.now() + 2 * 60 * 1000).toISOString(),
  };
...
export async function buildApplePassBuffer({ 
  ...
  max,
  relevantDate = null,
  latestMessage = null  // ⭐ NUEVO PARÁMETRO PARA MENSAJES
}) {
  try {
    ...
    const model = buildTempModelDir({
      ...
      max,
      relevantDate,
      latestMessage  // ⭐ Pasar el mensaje al modelo
    });
