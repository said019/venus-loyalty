      ...
      backFields: [
        {
          ...
          value: "Completa tus sellos y canjea un facial gratis.\n\nVálido en cualquier sucursal Venus Cosmetología.",
        },
        {
          key: "card_details",
          label: "Detalles de la Tarjeta",
          value: `Tarjeta: ${cardId}\nCliente: ${name || "Cliente"}\nSellos: ${stamps}/${max}`,
        },
      ],
    },

    // Colores de marca (ajustados para mejor contraste)
    backgroundColor: "rgb(205, 216, 166)", // verde claro Venus
    foregroundColor: "rgb(74, 85, 50)",    // verde oscuro para texto
    labelColor: "rgb(104, 115, 80)",       // verde medio para labels
...
export async function buildApplePassBuffer({ cardId, name, stamps, max }) {
  try {
    ...
    if (fs.existsSync(stripFile)) {
      ...
      console.warn(`[Apple Wallet] ⚠️  No existe ${stripFile}`);
    }

    if (fs.existsSync(strip2xFile)) {
      pass.addBuffer("strip@2x.png", fs.readFileSync(strip2xFile));
      console.log(`[Apple Wallet] ✓ Strip @2x agregado`);
    }
