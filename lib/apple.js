...
const IMAGE_SPECS = {
  ...
  strip: { 
    width: 624, 
    height: 168,
    description: "Strip dinámico con progreso de sellos",
    required: false
  }
};

/* =========================================================
   RUTAS DE IMÁGENES - CORREGIDO
   ========================================================= */
const PROJECT_ROOT = process.cwd();
const ASSETS_DIR = path.join(PROJECT_ROOT, "assets");

const IMAGE_PATHS = {
  logo: path.join(ASSETS_DIR, "logow.png"),    // Tu logo original
  icon: path.join(ASSETS_DIR, "logow.png"),    // Mismo logo para icono
};

/* =========================================================
   GENERADOR DE STRIP DINÁMICO
   ========================================================= */
async function generateDynamicStrip(stamps, maxStamps, customerName) {
  try {
    const width = 624;
    const height = 168;
    
    console.log(`[Strip Generator] Creando strip dinámico: ${stamps}/${maxStamps} sellos`);

    // Colores de la marca Venus
    const colors = {
      background: '#898e78',     // Verde principal
      completed: '#cdd8a6',      // Verde claro para sellos completados
      pending: '#6b705c',        // Verde oscuro para sellos pendientes
      text: '#fff6e3',           // Texto en marfil
      accent: '#a5a58d'          // Color de acento
    };
      ...
      <svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
        ...
        <defs>
          <linearGradient id="bgGradient" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <stop offset="100%" stop-color="#7a7f6a" />
          </linearGradient>
        </defs>
        ...
        <!-- Patrón sutil -->
        <circle cx="50" cy="50" r="40" fill="${colors.accent}" opacity="0.1" />
        <circle cx="${width-50}" cy="${height-50}" r="60" fill="${colors.accent}" opacity="0.1" />
        ...
        <!-- Barra de progreso -->
        <rect x="62" y="100" width="500" height="20" rx="10" fill="${colors.pending}" />
        <rect x="62" y="100" width="${(500 * stamps) / maxStamps}" height="20" rx="10" fill="${colors.completed}" />
        ...
        <text x="312" y="140" 
              ...
              font-weight="bold" 
              fill="${colors.text}">
          ${stamps} de ${maxStamps} sellos completados
        </text>
        ...
        <text x="312" y="160" 
              ...
              fill="${colors.text}"
              opacity="0.8">
          ${Math.round((stamps / maxStamps) * 100)}% completado
        </text>
      </svg>
    `;

    // Convertir SVG a PNG
    const stripBuffer = await sharp(Buffer.from(svgBackground))
      .png()
      .toBuffer();

    console.log(`[Strip Generator] ✅ Strip dinámico creado: ${stripBuffer.length} bytes`);
    return stripBuffer;

  } catch (error) {
    console.error('[Strip Generator] ❌ Error creando strip dinámico:', error);
    
    // Fallback: crear strip simple con colores
...
/* =========================================================
   GENERADOR ALTERNATIVO CON SELLOS VISUALES
   ========================================================= */
...
/* =========================================================
   ...
   ========================================================= */
async function processAppleWalletImage(buffer, type) {
  try {
    const specs = IMAGE_SPECS[type];
    if (!specs) {
      throw new Error(`Tipo de imagen no reconocido: ${type}`);
    }

    console.log(`[Image Processor] Procesando ${type} (${specs.width}x${specs.height})`);
    ...
    console.log(`[Image Processor] ✅ ${type} procesado: ${processedBuffer.length} bytes`);
    return processedBuffer;

  } catch (error) {
    console.error(`[Image Processor] ❌ Error procesando ${type}:`, error.message);
    throw error;
  }
}
...
function buildTempModelDir({
  ...
  // Solo crear fallbacks esenciales con nombres CORRECTOS para Apple Wallet
...
/* =========================================================
   GENERADOR PRINCIPAL DE PASE APPLE - CORREGIDO
   ========================================================= */
export async function buildApplePassBuffer({ cardId, name, stamps, max }) {
  try {
    ...
    const imageVerification = await verifyImages();
    ...
    // 5. AGREGAR IMÁGENES Y STRIP DINÁMICO - CORREGIDO
    console.log(`\n🖼️  AGREGANDO IMÁGENES...`);

    // Función para agregar imágenes procesadas
    const addProcessedImage = async (fileName, imagePath, imageType) => {
      if (!fs.existsSync(imagePath)) {
        console.log(`[Image Handler] ❌ No encontrado: ${fileName}`);
        return false;
      }

      try {
        const originalBuffer = fs.readFileSync(imagePath);
        const processedBuffer = await processAppleWalletImage(originalBuffer, imageType);
    ...
    // AGREGAR LOGO E ICONO CON NOMBRES CORRECTOS PARA APPLE WALLET
    // Logo (usa tu logow.png pero lo guarda como logo.png en el pase)
    await addProcessedImage("logo.png", IMAGE_PATHS.logo, "logo");
    await addProcessedImage("logo@2x.png", IMAGE_PATHS.logo, "logo");

    // Icono (usa tu logow.png pero lo guarda como icon.png en el pase)
    await addProcessedImage("icon.png", IMAGE_PATHS.icon, "icon");
    await addProcessedImage("icon@2x.png", IMAGE_PATHS.icon, "icon");

    // 6. GENERAR Y AGREGAR STRIP DINÁMICO
    console.log(`\n🎨 GENERANDO STRIP DINÁMICO...`);
    
    // Elegir el tipo de strip (barra de progreso o sellos visuales)
    const stripBuffer = await generateStampStrip(stamps, max); // Cambia a generateDynamicStrip si prefieres barra
    
    if (stripBuffer) {
      pass.addBuffer(stripBuffer, "strip.png");
      pass.addBuffer(stripBuffer, "strip@2x.png"); // Misma imagen para ambas resoluciones
    ...
    console.log(`   👤 Cliente: ${name}`);
    console.log(`   🖼️  Imágenes: Logo e Icono procesados desde logow.png`);
