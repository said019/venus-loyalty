...
export async function buildApplePassBuffer({ cardId, name, stamps, max }) {
  try {
    ...
    if (!stripFound) {
      console.warn(
        `[Apple Wallet] ⚠️  No se encontró ningún archivo de strip para ${safeStamps} sellos`
      );
      console.warn(
        `[Apple Wallet]     Buscado: ${stripVariants.join(", ")}`
      );
    }

    const strip2xFile = path.join(
      assetsDir,
      `stamp-strip-${safeStamps}@2x.png`
    );
    if (fs.existsSync(strip2xFile)) {
      pass.addBuffer("strip@2x.png", fs.readFileSync(strip2xFile));
      console.log(
        `[Apple Wallet] ✓ Strip @2x de alta resolución agregado`
      );
    }
