...
export async function buildApplePassBuffer({
  ...
  try {
    ...
    // (Render: APPLE_PASS_CERT / APPLE_PASS_KEY / APPLE_WWDR).
    // Default a los archivos versionados en el repo (todos RSA). OJO: el WWDR
    // por defecto es wwdr_rsa.pem (G4, RSA); wwdr.pem es G6/ECDSA y node-forge
    // NO lo soporta ("OID is not RSA").
    const signerCertPem = readPemFlexible(
      process.env.APPLE_SIGNER_CERT_PEM,
      process.env.APPLE_PASS_CERT || "./pass.pem",
      "APPLE_SIGNER_CERT_PEM / APPLE_PASS_CERT (pass.pem)",
      true
    );
    const signerKeyPem = readPemFlexible(
      process.env.APPLE_SIGNER_KEY_PEM,
      process.env.APPLE_PASS_KEY || "./pass.key",
      "APPLE_SIGNER_KEY_PEM / APPLE_PASS_KEY (pass.key)",
      false
    );
    const wwdrPem = readPemFlexible(
      process.env.APPLE_WWDR_CERT_PEM,
      process.env.APPLE_WWDR || "./wwdr_rsa.pem",
