...
function buildTempModelDir({
  ...
  // IMPORTANTE: tipo generic para controlar mejor layout
  const passJson = {
    formatVersion: 1,
    passTypeIdentifier: passTypeId,
    teamIdentifier: teamId,
    serialNumber: cardId,
    webServiceURL: process.env.BASE_URL + "/v1",
    authenticationToken: process.env.APPLE_AUTH_TOKEN,
    organizationName: orgName,
    description: "Tarjeta de Lealtad Venus",
    logoText: "Venus Cosmetología",

    // Tipo GENERIC: strip arriba, info abajo (como Airbnb)
    generic: {
      // Campo auxiliar (pequeño, arriba del nombre)
      auxiliaryFields: [
        {
          key: "program",
          label: "PROGRAMA",
      ...
      // Nombre del cliente (mediano, debajo del strip)
      secondaryFields: [
        {
          key: "name",
          label: "CLIENTE",
      ...
      // Sin primaryFields para que el nombre no quede gigante
      primaryFields: [],

      // Reverso con términos
      backFields: [
        {
          key: "terms",
    ...
    // Colores de tu mockup
    backgroundColor: "rgb(154, 159, 130)", // verde-gris Venus
...
export async function buildApplePassBuffer({ cardId, name, stamps, max }) {
  try {
    ...
    // 2) STRIP de sellos (más compacto)
    ...
    for (const variant of stripVariants) {
      ...
      if (fs.existsSync(stripFile)) {
        ...
        // Buscar y agregar versión @2x si existe
        const strip2xFile = stripFile.replace('.png', '@2x.png');
        if (fs.existsSync(strip2xFile)) {
          const buf2x = fs.readFileSync(strip2xFile);
          pass.addBuffer("strip@2x.png", buf2x);
          console.log(`[Apple Wallet] ✓ Strip agregado: ${variant} + @2x`);
        } else {
    ...
    if (!stripFound) {
      console.warn(
        `[Apple Wallet] ⚠️  No se encontró strip para ${safeStamps} sellos`
      );
      console.warn(`[Apple Wallet]     Ejecuta: node scripts/generateStampStrips.js`);
    }
