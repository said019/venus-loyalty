...
function buildTempModelDir({ orgName, passTypeId, teamId }) {
  // Si ya diste una carpeta real, úsala tal cual
  const modelFromEnv = process.env.APPLE_PASS_MODEL_DIR;
  if (modelFromEnv && fs.existsSync(modelFromEnv)) return modelFromEnv;

  // Algunas versiones de passkit-generator requieren que la carpeta termine en .pass
  ...
  // Archivos mínimos requeridos dentro de la carpeta .pass
  fs.writeFileSync(path.join(dir, "pass.json"), JSON.stringify(passJson, null, 2));
  fs.writeFileSync(path.join(dir, "icon.png"), TINY_PNG);
  fs.writeFileSync(path.join(dir, "icon@2x.png"), TINY_PNG);
  fs.writeFileSync(path.join(dir, "logo.png"), TINY_PNG);
  fs.writeFileSync(path.join(dir, "logo@2x.png"), TINY_PNG);
