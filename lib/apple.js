...
function getPassClass() {
  ...
  // Algunos bundlers ponen todo en default
  if (PassLib && typeof PassLib.default === "function") return PassLib.default;
  if (
    PassLib &&
    PassLib.default &&
    typeof PassLib.default.PKPass === "function"
  )
    return PassLib.default.PKPass;

  // Último recurso, por si exportan directamente una función
...
function buildTempModelDir({
  ...
  const passJson = {
    ...
    // Tipo storeCard (estructura nativa de Apple)
    storeCard: {
      headerFields: [
        {
          key: "program",
    ...
    // Colores de marca
    backgroundColor: "rgb(205,216,166)", // verde claro #cdd8a6
    foregroundColor: "rgb(255,246,227)", // crema #fff6e3
...
export async function buildApplePassBuffer({ cardId, name, stamps, max }) {
  try {
    ...
    // 1) Logo principal (mismo que usas en la web)
    const logoPath = path.join(assetsDir, "logo.png");
    if (fs.existsSync(logoPath)) {
      const buf = fs.readFileSync(logoPath);
      // OJO: firma correcta addBuffer(nombre, buffer)
    ...
    // 2) Fondo opcional (hero) si quieres algo además del color
    const heroPath = path.join(assetsDir, "hero.png");
    if (fs.existsSync(heroPath)) {
      const buf = fs.readFileSync(heroPath);
      pass.addBuffer("background.png", buf);
      pass.addBuffer("background@2x.png", buf);
      console.log("[Apple Wallet] Fondo personalizado agregado");
    } else {
      console.log("[Apple Wallet] Sin hero.png, se usa sólo color de fondo");
    }

    // 3) Strip dinámico con los sellos (imagen precompuesta)
    // Necesitas:
    //   public/assets/stamp-strip-0.png
    //   public/assets/stamp-strip-1.png
    //   ...
    //   public/assets/stamp-strip-8.png  (o hasta tu max)
    const safeMax = Number(max) || 8;
    const safeStamps = Math.max(0, Math.min(Number(stamps) || 0, safeMax));
    const stripIndex = safeStamps; // 0..max
    const stripFile = path.join(assetsDir, `stamp-strip-${stripIndex}.png`);
