...
/* =============== Resolver clase de passkit-generator =============== */
...
/**
 * Importante: aquí ya metemos TODOS los datos dinámicos
 * (nombre, sellos, código QR, etc.) en el pass.json,
 * para no depender de las APIs raras de la librería.
 */
function buildTempModelDir({
  ...
  const passJson = {
    ...
    // Código de barras (forma antigua) – muchas libs lo convierten a "barcodes"
    barcode: {
...
/* =============== Exportar buffer (distintas APIs) =============== */
async function exportPassToBuffer(pass) {
  ...
  const keys = Object.getOwnPropertyNames(
    Object.getPrototypeOf(pass)
  ).filter((k) => typeof pass[k] === "function");
  throw new Error(
...
export async function buildApplePassBuffer({ cardId, name, stamps, max }) {
  ...
  // (ya no tocamos campos ni barcodes aquí; todo viene de pass.json)

  // 4) Imágenes personalizadas (si las configuras en env)
  const addImg = (slot, p) => {
  ...
  // 5) Exportar a Buffer y regresar
