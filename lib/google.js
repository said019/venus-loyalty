// lib/google.js - VERSIÓN COMPLETA CON TODOS LOS EXPORTS
 ...
 * (solo se hace 1 vez; luego queda reutilizable)
 */
export async function createLoyaltyClass() {
  try {
    ...
    console.log("[GOOGLE API] Intentando crear clase:", classId);
    console.log("[GOOGLE API] Usando issuerId:", issuerId);
    console.log("[GOOGLE API] Token length:", token.length);

    const loyaltyClass = {
      id: classId,

      // Se muestra pequeño arriba a la izquierda
      issuerName: "Venus Cosmetología",

      // Nombre del programa (texto en la parte superior de la tarjeta)
      programName: "Venus Lealtad",

      // Logo del programa (el circulito que ya viste)
      programLogo: {
        sourceUri: {
      ...
      // FONDO: verde tipo Apple (equivalente al #898e78 que usamos en el pase)
      hexBackgroundColor: "#898e78",
    ...
    console.log(
      ...
      JSON.stringify(loyaltyClass, null, 2)
    );
...
export async function updateLoyaltyObject(cardId, name, stamps, max) {
  try {
    ...
    const objectId = `${issuerId}.${cardId.replace(
      /[^a-zA-Z0-9._+-]/g,
      "_"
    )}`;

    const safeStamps = Number.parseInt(stamps ?? 0, 10) || 0;
    const safeMax = Number.isInteger(max) ? max : 8;
    ...
    const loyaltyObject = {
      ...
      // Lo que ves en la pildorita blanca
      accountId: cardId,

      // Texto grande (similar a Apple): nombre del cliente
 ...
 * (igual lógica, solo cambiamos cómo se muestran los sellos)
 */
...
/**
 * Función de prueba para crear objeto de prueba
 */
