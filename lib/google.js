 ...
 */
// Normaliza la private_key: soporta base64 (sin headers PEM) y saltos de
// línea escapados (\n, \r\n, \\n). Devuelve PEM con saltos reales.
function normalizePrivateKey(raw) {
  let k = String(raw || "").trim();
  if (!k) return "";
  // Si viene en base64 (sin headers PEM), decodificar.
...
export function loadServiceAccount() {
  // 1) JSON COMPLETO en base64 (Railway, recomendado): GOOGLE_SA_JSON_B64.
  //    Una sola línea, sin problemas de escapado.
  if (process.env.GOOGLE_SA_JSON_B64) {
    try {
      ...
      if (json.client_email && json.private_key) {
        console.log("[GOOGLE] Credenciales desde GOOGLE_SA_JSON_B64");
        return {
      ...
      console.error("[GOOGLE] GOOGLE_SA_JSON_B64 inválido:", e.message);
    }
  }

  // 2) Inline: GOOGLE_SA_EMAIL + GOOGLE_SA_PRIVATE_KEY (acepta base64 o PEM).
  const client_email = process.env.GOOGLE_SA_EMAIL;
  const private_key = normalizePrivateKey(process.env.GOOGLE_SA_PRIVATE_KEY);
  if (client_email && private_key) {
    console.log("[GOOGLE] Credenciales inline (GOOGLE_SA_EMAIL + KEY)");
    return { client_email, private_key };
  }

  // 3) Archivo (legacy / Render).
  ...
  if (fs.existsSync(filePath)) {
    ...
    try {
      ...
      if (json.client_email && json.private_key) {
        return { client_email: json.client_email, private_key: normalizePrivateKey(json.private_key) };
      }
      console.error("[GOOGLE] El archivo no contiene client_email/private_key.");
    } catch (e) {
      console.error("[GOOGLE] Archivo de credenciales inválido (no es JSON):", e.message);
    }
  }

  throw new Error(
    "Credenciales de Service Account no encontradas. Configura GOOGLE_SA_JSON_B64 (recomendado), o GOOGLE_SA_EMAIL + GOOGLE_SA_PRIVATE_KEY, o el archivo."
  );
}
