 ...
 */
export async function updateLoyaltyObject(cardId, name, stamps, max, cardType = 'loyalty') {
  try {
    const token = await getWalletAccessToken();
    const issuerId = process.env.GOOGLE_ISSUER_ID;
    const baseUrl = normalizeBaseUrl(process.env.BASE_URL);
    ...
    const loyaltyObject = {
      ...
      loyaltyPoints: {
        ...
        label: cardType === 'massage' ? "SESIONES" : "SELLOS",
      },
      ...
      // Textos
      textModulesData: cardType === 'massage' ? [
        {
          id: "customer_name",
          header: "Cliente",
        ...
        },
        {
          ...
          body: stamps >= 10
            ? "🎁🎁 ¡2 regalos canjeados!"
            : stamps >= 5
              ? "🎁 ¡1er regalo listo! — Próximo en sesión 10"
              : `🎁 Próximo regalo en sesión 5 (faltan ${5 - stamps})`,
        },
      ] : [
        {
          id: "customer_name",
      ...
      imageModulesData: [
        {
          ...
          mainImage: {
            sourceUri: {
              uri: `${baseUrl}/assets/${stripPrefix}-${stamps}.png`,
            },
            contentDescription: {
              defaultValue: {
                language: "es",
                value: cardType === 'massage'
                  ? `Sesiones: ${stamps} de ${max} — 🎁 Regalos en sesión 5 y 10`
                  : `Progreso: ${stamps} de ${max} sellos`,
              },
            },
          },
        },
      ],
      // Color de fondo según tipo de tarjeta
      hexBackgroundColor: cardType === 'massage' ? '#C4936E' : undefined,
...
export function buildGoogleSaveUrl({ cardId, name, stamps, max, cardType = 'loyalty' }) {
  const creds = loadServiceAccount();
      ...
      loyaltyObjects: [
        {
              ...
              mainImage: {
                ...
                contentDescription: {
                  defaultValue: {
                    ...
                    value: cardType === 'massage'
                      ? `Sesiones: ${stamps} de ${max} — 🎁 Regalos en sesión 5 y 10`
                      : `${stamps} de ${max} sellos completados`,
                  },
                },
              },
            },
          ...
          // Color de fondo para masajes (terracota)
          ...(cardType === 'massage' ? { hexBackgroundColor: '#C4936E' } : {}),
