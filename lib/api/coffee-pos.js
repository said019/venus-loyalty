...
router.get('/sales', requireRole("admin"), async (req, res) => {
  try {
    ...
    const where = {};
    // Rango en día natural de México (antes se usaba UTC: las ventas después de
    // las 18:00 se contaban en el día siguiente).
    if (from || to) {
      where.createdAt = {};
      if (from) where.createdAt.gte = startOfDayMexico(from);
      if (to) where.createdAt.lte = endOfDayMexico(to);
    }
    if (cashier) where.cashierName = cashier;
    if (status) where.status = status;

    // El tope fijo de 200 truncaba EN SILENCIO el reporte de Ventas: un mes con
    // más de 200 tickets de café perdía los más viejos y el total salía corto.
    // Con rango de fechas se trae todo el periodo; sin rango se conserva el tope.
    const take = Math.min(parseInt(req.query.limit, 10) || (from || to ? 5000 : 200), 5000);
    ...
    if (sales.length === take) {
      console.warn(`[POS] /sales alcanzó el tope de ${take} registros (from=${from} to=${to}): el resultado puede estar truncado`);
    }
...
router.post('/sales/:id/cancel', requireRole("admin"), async (req, res) => {
  try {
    ...
    const antes = await prisma.coffeeSale.findUnique({ where: { id: req.params.id } });
    if (!antes) return res.status(404).json({ success: false, error: 'Venta no encontrada' });
    const sale = await prisma.coffeeSale.update({
      where: { id: req.params.id },
      data: { status: 'cancelled', cancelReason: reason, cancelledBy: req.admin.email || 'admin' },
    });

    // A la bitácora de correcciones de la Caja, junto con las de citas e
    // ingresos: así "Correcciones del día" enseña todo lo que se quitó.
    try {
      await prisma.cajaEdit.create({
        data: {
          ...
          entityId: antes.id,
          action: 'delete',
          oldValue: { clientName: antes.folio || 'Coffee Bar', serviceName: 'Café', total: antes.total, paymentMethod: antes.paymentMethod },
          newValue: null,
      ...
      console.error('[POS] no se pudo anotar la cancelación en la bitácora:', e.message);
    }
...
router.get('/reports/daily', requireRole("admin"), async (req, res) => {
  try {
    ...
    const start = startOfDayMexico(dateStr);
    const end = endOfDayMexico(dateStr);
