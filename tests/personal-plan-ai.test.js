...
test('blocked recipe never calls Claude',async()=>{let called=false;const ask=createPersonalPlanAI({generate:async()=>{called=true;}});const r=await ask({recipeId:'c0',index:6,question:'Pechuga'});assert.equal(called,false);assert.equal(r.usedAI,false);assert.deepEqual(r.suggestions,[]);});
