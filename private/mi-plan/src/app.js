...
import {dateKey,isDateKey,proposeSwap,effectiveIngredients,shoppingList,formatAmount,weightState} from './engine.js';
...
function motivoNoAplica(result,original){
 ...
 if(r.startsWith('Tu plan pone'))return 'Tu plan la pone en dos subgrupos; no se cambia';
...
async function askClaude(button){
 ...
 const index=Number(button.dataset.index),recipeId=current.id;button.disabled=true;answer.textContent='Buscando entre las opciones del mismo grupo…';
 ...
 try{
  ...
  answer.innerHTML='<p>'+esc(data.message)+'</p>'+data.suggestions.map(s=>'<button class="candidate" data-target="'+esc(s.id)+'" data-index="'+index+'">'+esc(s.name)+'<small>'+esc(qty(s)+peso(foods[s.id]))+(s.source==='SMAE'?' · SMAE, no viene de tu plan':'')+' · Revisar cambio</small></button>').join('');
