...
import {dateKey,isDateKey,proposeSwap,effectiveIngredients,shoppingList,formatAmount,weightState,checkAIProposal} from './engine.js';
...
const qty=i=>i.quantity==null?'Cantidad no indicada':formatAmount(i.quantity,i.unit);
// El estado va pegado al peso en todos lados: receta, cambio, Claude, compras y texto copiado.
const peso=f=>{const s=weightState(f);return s?' · '+s:'';};
const r1=n=>Math.round(n*10)/10;
const aporteTxt=n=>`${Math.round(n.kcal)} kcal · ${r1(n.protein)} g proteína · ${r1(n.fat)} g grasa · ${r1(n.carbs)} g hidratos`;
const AVISO_COCIDO='Las carnes marcadas «peso ya cocido» se pesan después de cocinarlas. En crudo pesan más porque al cocinarse pierden agua: como referencia, el USDA mide que la pechuga y la res conservan entre 70 y 80 % de su peso. Compra un poco más de lo que dice la lista; la app no convierte la cantidad.';
...
function renderShopping(){
 ...
 app.innerHTML=frame('Lista de compras','Selecciona los días que vas a preparar.')+`<div class="actions"><button id="shop-week">Seleccionar toda la semana</button><button id="shop-clear">Quitar selección</button></div><p><strong>Total para ${shoppingDays.size} días · ${entries.length} comidas</strong></p><p class="muted">${[...shoppingDays].sort().map(d=>esc(pretty(d))).join(' · ')}</p><div class="days-list">${dates().map(d=>`<label><input type="checkbox" data-shop-day="${d}" ${shoppingDays.has(d)?'checked':''}>${esc(new Date(d+'T12:00:00').toLocaleDateString('es-MX',{weekday:'short',day:'numeric'}))}</label>`).join('')}</div><p class="muted">Marca lo que ya tienes en casa. Los cambios pendientes no modifican esta lista.</p>${entries.some(e=>e.recipe.warning)?'<div class="warning">Algunas recetas tienen aclaraciones pendientes. La lista suma las cantidades del plan y los cambios confirmados; no corrige las diferencias del documento.</div>':''}${rows.some(x=>weightState(foods[x.foodId])==='peso ya cocido')?'<div class="warning">'+esc(AVISO_COCIDO)+'</div>':''}${!rows.length?'<p class="empty">Elige al menos un día para reunir sus ingredientes.</p>':['Ingredientes','Opcionales'].map((title,k)=>`<section class="section"><h3>${title}</h3>${rows.filter(x=>Boolean(x.optional)===Boolean(k)).map(row=>`<label class="shopping-row"><input type="checkbox" data-pantry="${esc(row.key)}" ${store.data.pantry[row.key]===true?'checked':''}><span>${esc(row.name)}${weightState(foods[row.foodId])?'<small> · '+esc(weightState(foods[row.foodId]))+'</small>':''}</span><em>Total: ${esc(qty(row))}</em></label>`).join('')||'<small>No hay ingredientes en este grupo.</small>'}</section>`).join('')}<div class="actions"><button class="primary" id="copy-list" ${rows.length?'':'disabled'}>Copiar lista</button></div><textarea id="copy-fallback" aria-label="Lista para copiar" hidden></textarea>`;
}
function renderPlan(){
...
function openRecipe(id){
 ...
 content.innerHTML=`<div class="dialog-top"><div><div class="eyebrow">${slots[r.slot][0]} · ${slots[r.slot][1]}</div><h2 id="dialog-title">${esc(r.title)}</h2></div><button class="close" data-close aria-label="Cerrar">×</button></div><p class="muted">${esc(pretty(day))} · ${changed?'Porción de tu plan, con un cambio confirmado':'Porción indicada en tu plan'}</p>${r.warning?`<div class="warning">${esc(r.warning)} Esta aclaración no bloquea los ingredientes con equivalencias comprobadas.</div>`:''}<h3>Ingredientes</h3>${list.map((i,index)=>`<div class="ingredient"><div><strong>${esc(foods[i.foodId]?.name||i.foodId)}</strong><span>${esc(qty(i)+peso(foods[i.foodId]))}${i.optional?' · Opcional':''}</span>${i.hold&&!i.swapped?'<small>Tu plan la pone en dos subgrupos: el cambio directo no aplica; Claude sí puede proponerte uno.</small>':''}${i.byAI?'<small>Cantidad propuesta por Claude y revisada con la tabla del SMAE.</small>':''}${i.swapped?'<small>Cambio confirmado para este día</small>':''}</div><button data-swap="${index}">Cambiar</button></div>`).join('')}${changed?'<div class="warning">La preparación siguiente corresponde a la receta original. Puede requerir adaptación por el cambio de ingrediente; no es una receta nueva validada.</div>':''}<section class="section"><h3>Preparación resumida</h3>${r.steps.length?`<ol class="steps">${r.steps.map(s=>`<li>${esc(s)}</li>`).join('')}</ol>`:'<p>El documento no incluye pasos de preparación para esta opción.</p>'}</section><details class="source"><summary>Ver procedencia de las cantidades</summary><p>Receta: página ${r.page}. Menú: páginas 8–10.</p>${list.map(i=>`<p>${esc(foods[i.foodId].name)}: ${esc(qty(i))}${i.byAI?' (cantidad de Claude)':''}. Equivalencia: ${esc(procedencia(foods[i.foodId]))}. Preparación/estado: ${esc(foods[i.foodId].prep)}${foods[i.foodId].prepFrom?' — '+esc(foods[i.foodId].prepFrom):''}.</p>`).join('')}</details><div class="actions"><button class="primary" data-done="${r.id}">${store.data.done[day]?.[r.id]?'Desmarcar comida':'Marcar como comido'}</button>${changed?'<button id="restore">Restaurar original</button>':''}</div>`;showModal();
}
// Por qué una opción no se puede aplicar, en palabras. Antes todo decía
// "Por confirmar", que sonaba a dato pendiente cuando casi siempre significa
// que el alimento es de otro grupo y por definición no equivale.
function motivoNoAplica(result,original){
 const r=result.reason||'';
 if(r.startsWith('Los alimentos pertenecen'))return 'No equivale a '+(groupNames[foods[original.foodId].group]||'este grupo');
 if(r.startsWith('Tu plan pone'))return 'Tu plan la pone en dos subgrupos; pídeselo a Claude';
 if(r.startsWith('Falta confirmar'))return 'Falta saber si el peso es crudo o cocido';
 if(r.startsWith('La unidad'))return 'Tu receta lo mide distinto que la tabla';
 if(r.startsWith('Se necesita'))return 'Tu receta no trae la cantidad';
 return r||'Por confirmar';
}
// De dónde sale cada cantidad. El plan manda; SMAE solo amplía opciones.
function procedencia(f){
 if(!f)return 'sin dato';
 if(f.source==='SMAE')return (f.ref||'SMAE')+' (referencia, no viene de tu plan)';
 return f.page?'página '+f.page+' de tu plan':'sin equivalencia específica';
}
function chooseSwap(index){
 if(!current?.ingredients[index])return;
 content.innerHTML='<div class="dialog-top"><h2 id="dialog-title">¿Qué se te antoja?</h2><button data-back>Volver</button></div><p>Cambiar: <strong>'+esc(foods[current.ingredients[index].foodId].name)+'</strong></p><details><summary>Ayúdame con Claude · opcional</summary><label for="ai-question">Describe lo que prefieres</label><textarea id="ai-question" class="search" maxlength="400" placeholder="Quiero una fruta cítrica"></textarea><label class="consent-row"><input id="ai-consent" type="checkbox"><span>Enviar a Claude el texto que escriba, este ingrediente y la lista de alimentos del catálogo. Claude propone la cantidad y la app la compara con la tabla del SMAE. No se adjuntan historial ni PDF completo. Evita escribir datos personales.</span></label><button class="primary" id="ask-ai" data-index="'+index+'">Buscar opciones con Claude</button><div id="ai-answer" role="status"></div></details><div class="section"><label for="search">Ver equivalencias sin IA</label><input id="search" class="search" placeholder="Por ejemplo: pechuga de pollo" autocomplete="off"><div id="candidates"></div></div>';
 ...
 const search=content.querySelector('#search');const update=()=>{
  ...
  content.querySelector('#candidates').innerHTML=candidates.map(({f,result})=>'<button class="candidate" data-target="'+f.id+'" data-index="'+index+'">'+esc(f.name)+(saved.includes(f.id)?' · Guardada':'')+'<small>'+esc(groupNames[f.group]||f.group)+' · '+(result.status==='valid'&&!current.blockSwaps?esc(qty(result)+peso(f)):esc(motivoNoAplica(result,current.ingredients[index])))+(f.source==='SMAE'?' · SMAE':'')+'</small></button>').join('')||'<p class="empty">No está en el catálogo verificado. No calcularemos una porción inventada.</p>';
...
async function askClaude(button){
 ...
 const index=Number(button.dataset.index),recipeId=current.id;button.disabled=true;answer.textContent='Claude está buscando opciones y cantidades…';
 ...
 try{
  ...
  answer.innerHTML='<p>'+esc(data.message)+'</p>'+data.suggestions.map(s=>'<button class="candidate" data-target="'+esc(s.id)+'" data-ai-qty="'+esc(s.quantity)+'" data-index="'+index+'">'+esc(s.name)+'<small>'+esc(qty(s)+peso(foods[s.id]))+(s.source==='SMAE'?' · SMAE, no viene de tu plan':'')+' · Revisar cambio</small>'+(s.after?'<small>Aporta ≈ '+esc(s.after.kcal)+' kcal y '+esc(s.after.proteina)+' g de proteína; el original, '+esc(s.before.kcal)+' kcal y '+esc(s.before.proteina)+' g.</small>':'')+'</button>').join('');
...
function proposal(index,target,aiQty){
 const original=current.ingredients[index];if(!original||!foods[target])return;
 if(aiQty!==undefined)return proposalAI(index,target,aiQty);
 const calc=proposeSwap(original,target,foods),result=current.blockSwaps?{status:'pending',reason:current.warning}:calc;
 const valid=result.status==='valid';
 content.innerHTML=`<div class="dialog-top"><h2 id="dialog-title">Revisa el cambio</h2><button data-back>Volver</button></div><div class="compare"><div><small>Original</small><strong>${esc(foods[original.foodId].name)}</strong>${esc(qty(original)+peso(foods[original.foodId]))}</div><div><small>Tu elección</small><strong>${esc(foods[target].name)}</strong>${valid?esc(qty(result)+peso(foods[target])):'Cantidad pendiente de confirmar'}</div></div><div class="${valid?'success':'warning'}">${valid?'Conserva los equivalentes del mismo grupo: '+esc(groupNames[foods[target].group])+'.':esc(result.reason)}</div><p class="source">Tabla: ${esc(procedencia(foods[original.foodId]))} y ${esc(procedencia(foods[target]))}. ${valid?'Solo se cambia este ingrediente; no se añade aceite ni se modifican acompañamientos.':'No se aplicará al menú ni a compras. Puedes guardar una nota para consultarla con tu nutrióloga; no hay revisión automática.'}</p><div class="actions"><button class="primary" data-confirm="${target}" data-index="${index}">${valid?'Confirmar solo para este día':'Guardar duda para mi nutrióloga'}</button>${valid?`<button data-alternative="${target}" data-index="${index}">Guardar como alternativa</button>`:''}</div>`;
}
// Cambio propuesto por Claude: la cantidad es suya; el aporte lo calcula la app
// con la tabla del SMAE y se muestra lado a lado para decidir con los números.
function proposalAI(index,target,quantity){
 const original=current.ingredients[index],check=checkAIProposal(original,target,quantity,foods),f=foods[target],ok=check.ok&&!current.blockSwaps;
 const fila=(t,n)=>n?`<p><small>${t}</small><br>${esc(aporteTxt(n))}</p>`:'';
 content.innerHTML=`<div class="dialog-top"><h2 id="dialog-title">Revisa el cambio</h2><button data-back>Volver</button></div><div class="compare"><div><small>Original</small><strong>${esc(foods[original.foodId].name)}</strong>${esc(qty(original)+peso(foods[original.foodId]))}</div><div><small>Propuesta de Claude</small><strong>${esc(f.name)}</strong>${esc(qty({quantity,unit:f.unit})+peso(f))}</div></div><div class="${ok?'success':'warning'}">${ok?(check.sameFamily?'Mismo tipo de alimento: se conservan los equivalentes. La grasa y las calorías pueden cambiar; aquí abajo están calculadas con la tabla del SMAE.':'Otro tipo de alimento: no es un equivalente, se igualó la energía. El aporte está calculado con la tabla del SMAE.'):esc(check.reason)}</div>${fila('Aporte del original',check.before)}${fila('Aporte de la propuesta',check.after)}${ok&&check.before.protein>=5&&check.after.protein<check.before.protein/2?`<div class="warning">Ojo: esta propuesta trae ${esc(r1(check.before.protein-check.after.protein))} g menos de proteína que el original.</div>`:''}<p class="source">Tabla: ${esc(procedencia(foods[original.foodId]))} y ${esc(procedencia(f))}. ${ok?'Solo se cambia este ingrediente; no se añade aceite ni se modifican acompañamientos.':'No se aplicará al menú ni a compras.'}</p><div class="actions">${ok?`<button class="primary" data-confirm="${esc(target)}" data-qty="${esc(quantity)}" data-index="${index}">Confirmar solo para este día</button>`:''}</div>`;
}
function confirmSwap(index,target,aiQty){
 if(aiQty!==undefined){
  if(!current?.ingredients[index]||current.blockSwaps||!checkAIProposal(current.ingredients[index],target,aiQty,foods).ok)return;
  const d=store.data.changes[day]??={};(d[current.id]??={})[index]={target,quantity:aiQty,by:'claude'};delete store.data.pending[day]?.[current.id+':'+index];
  toast('Cambio confirmado solo para este día.');persist();render();openRecipe(current.id);return;
 }
...
document.addEventListener('click',async e=>{
 ...
 if(b.dataset.target){proposal(Number(b.dataset.index),b.dataset.target,b.dataset.aiQty!==undefined?Number(b.dataset.aiQty):undefined);return;}
 if(b.dataset.confirm){confirmSwap(Number(b.dataset.index),b.dataset.confirm,b.dataset.qty!==undefined?Number(b.dataset.qty):undefined);return;}
 ...
 if(b.id==='copy-list'){const rows=shoppingList([...shoppingDays].flatMap(d=>menuForDate(d).map(r=>({recipe:r,changes:store.data.changes[d]?.[r.id]||{}}))),foods);const pending=rows.filter(r=>store.data.pantry[r.key]!==true);const text='Lista de compras · Total para '+shoppingDays.size+' días\n'+pending.map(r=>`${r.name}: ${qty(r)}${peso(foods[r.foodId])}${r.optional?' (opcional)':''}`).join('\n')+(pending.some(r=>weightState(foods[r.foodId])==='peso ya cocido')?'\n\n'+AVISO_COCIDO:'');try{await navigator.clipboard.writeText(text);toast('Lista copiada, sin lo que ya tienes.');}catch{const box=document.querySelector('#copy-fallback');box.hidden=false;box.value=text;box.focus();box.select();toast('Selecciona y copia la lista.');}}
});
