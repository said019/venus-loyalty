...
function procedencia(f){
 ...
 if(f.source==='SMAE')return 'SMAE 4ª ed. (referencia, no viene de tu plan)';
...
function chooseSwap(index){
 ...
 const search=content.querySelector('#search');const update=()=>{
  ...
  content.querySelector('#candidates').innerHTML=candidates.map(({f,result})=>'<button class="candidate" data-target="'+f.id+'" data-index="'+index+'">'+esc(f.name)+(saved.includes(f.id)?' · Guardada':'')+'<small>'+esc(groupNames[f.group]||f.group)+' · '+(result.status==='valid'&&!current.blockSwaps?esc(qty(result)):esc(motivoNoAplica(result,current.ingredients[index])))+(f.source==='SMAE'?' · SMAE':'')+'</small></button>').join('')||'<p class="empty">No está en el catálogo verificado. No calcularemos una porción inventada.</p>';
