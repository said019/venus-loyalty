...
function selectDay(value){day=value;shoppingDays=new Set([value]);}
...
function renderShopping(){
 ...
 app.innerHTML=frame('Lista de compras','Selecciona los días que vas a preparar.')+`<div class="days-list">${dates().map(d=>`<label><input type="checkbox" data-shop-day="${d}" ${shoppingDays.has(d)?'checked':''}>${esc(new Date(d+'T12:00:00').toLocaleDateString('es-MX',{weekday:'short',day:'numeric'}))}</label>`).join('')}</div><p class="muted">Marca lo que ya tienes en casa. Los cambios pendientes no modifican esta lista.</p>${entries.some(e=>e.recipe.warning)?'<div class="warning">Algunas recetas tienen aclaraciones pendientes. La lista conserva sus cantidades originales, no corrige esas diferencias.</div>':''}${!rows.length?'<p class="empty">Elige al menos un día para reunir sus ingredientes.</p>':['Ingredientes','Opcionales'].map((title,k)=>`<section class="section"><h3>${title}</h3>${rows.filter(x=>Boolean(x.optional)===Boolean(k)).map(row=>`<label class="shopping-row"><input type="checkbox" data-pantry="${esc(row.key)}" ${store.data.pantry[row.key]===true?'checked':''}><span>${esc(row.name)}${row.prep==='unknown'?'<small> · Pesaje por aclarar</small>':''}</span><em>${esc(qty(row))}</em></label>`).join('')||'<small>No hay ingredientes en este grupo.</small>'}</section>`).join('')}<div class="actions"><button class="primary" id="copy-list" ${rows.length?'':'disabled'}>Copiar lista</button></div><textarea id="copy-fallback" aria-label="Lista para copiar" hidden></textarea>`;
}
function renderPlan(){
 ...
 app.innerHTML=`<div class="eyebrow">TU ESPACIO PERSONAL</div><h1>Mi plan</h1><p class="plan-note">Esta es una transcripción de tu plan, no una dieta nueva. Las equivalencias se calculan con la tabla del documento; no se generan con inteligencia artificial.</p><section class="section"><h3>Solicitudes por confirmar</h3>${pendings.length?pendings.map(({date,key,p})=>`<div class="request"><strong>${esc(foods[p.target].name)}</strong><p>${esc(pretty(date))} · ${esc(p.reason)}</p><button data-remove-pending="${esc(key)}" data-pending-date="${date}">Quitar solicitud</button></div>`).join(''):'<p class="muted">Aquí se guardan los cambios que necesitan aclaración antes de aplicarse.</p>'}</section><section class="section"><h3>Alternativas guardadas</h3>${store.data.alternatives.filter(a=>recipe(a.recipeId)&&foods[a.target]).map(a=>`<p>${esc(recipe(a.recipeId).title)} → ${esc(foods[a.target].name)} <small>Se vuelve a verificar al elegirla desde la receta.</small></p>`).join('')||'<p class="muted">Guarda una alternativa desde la pantalla de cambio. No modifica días futuros.</p>'}</section><section class="section"><h3>Aclaraciones del documento</h3><div class="warning">Líquidos: la página 7 indica 2–2.5 L y la 26 indica 2.5–3 L. La meta permanece sin definir hasta aclararlo con tu nutrióloga.</div><p>En las recetas marcadas, encontrarás diferencias entre ingredientes, preparación o distribución de equivalentes. Se conserva el original y se bloquean sus modificaciones.</p></section><section class="section"><h3>Privacidad y almacenamiento</h3><p class="plan-note">Tu cuenta de administrador de Venus protege el acceso a este plan. Tus registros se guardan solo en este navegador: no se sincronizan con otros dispositivos ni se envían a una IA. No es una cuenta cifrada: otra persona que use este perfil del navegador podría verlos. Borrar los datos del navegador elimina tus registros.</p><button id="clear" class="danger">Borrar mis registros</button></section><section class="section"><h3>Procedencia</h3><p>Páginas del documento: distribución 4; menú 8–10; recetas 11–20; equivalencias 21–25. Los pasos se presentan resumidos. El PDF no se publica en esta app.</p></section>`;
}
...
function openRecipe(id){
 ...
 content.innerHTML=`<div class="dialog-top"><div><div class="eyebrow">${slots[r.slot][0]} · ${slots[r.slot][1]}</div><h2 id="dialog-title">${esc(r.title)}</h2></div><button class="close" data-close aria-label="Cerrar">×</button></div><p class="muted">${esc(pretty(day))} · Porción indicada en tu plan</p>${r.warning?`<div class="warning">${esc(r.warning)} Los cambios de esta receta están pendientes de revisión.</div>`:''}<h3>Ingredientes</h3>${list.map((i,index)=>`<div class="ingredient"><div><strong>${esc(foods[i.foodId]?.name||i.foodId)}</strong><span>${esc(qty(i))}${i.optional?' · Opcional':''}</span>${foods[i.foodId]?.prep==='unknown'?'<small>No se especifica crudo/cocido.</small>':''}${i.swapped?'<small>Cambio confirmado para este día</small>':''}</div><button data-swap="${index}">Cambiar</button></div>`).join('')}${changed?'<div class="warning">La preparación siguiente corresponde a la receta original. Puede requerir adaptación por el cambio de ingrediente; no es una receta nueva validada.</div>':''}<section class="section"><h3>Preparación resumida</h3>${r.steps.length?`<ol class="steps">${r.steps.map(s=>`<li>${esc(s)}</li>`).join('')}</ol>`:'<p>El documento no incluye pasos de preparación para esta opción.</p>'}</section><details class="source"><summary>Ver procedencia de las cantidades</summary><p>Receta: página ${r.page}. Menú: páginas 8–10.</p>${r.ingredients.map(i=>`<p>${esc(foods[i.foodId].name)}: ${esc(qty(i))}. ${foods[i.foodId].page?'Equivalencia: página '+foods[i.foodId].page+'.':'Sin equivalencia específica.'} Preparación/estado: ${esc(foods[i.foodId].prep)}.</p>`).join('')}</details><div class="actions"><button class="primary" data-done="${r.id}">${store.data.done[day]?.[r.id]?'Desmarcar comida':'Marcar como comido'}</button>${changed?'<button id="restore">Restaurar original</button>':''}</div>`;showModal();
}
function chooseSwap(index){
 if(!current?.ingredients[index])return;
 content.innerHTML=`<div class="dialog-top"><h2 id="dialog-title">¿Qué se te antoja?</h2><button data-back>Volver</button></div><p>Cambiar: <strong>${esc(foods[current.ingredients[index].foodId].name)}</strong></p><label for="search">Busca un alimento del catálogo</label><input id="search" class="search" placeholder="Por ejemplo: pechuga de pollo" autocomplete="off"><div id="candidates"></div>`;
 const search=content.querySelector('#search');const update=()=>{const q=search.value.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();const saved=store.data.alternatives.filter(a=>a.recipeId===current.id&&a.index===index).map(a=>a.target);const candidates=Object.values(foods).filter(f=>f.portion&&f.id!==current.ingredients[index].foodId&&f.name.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().includes(q)).sort((a,b)=>Number(saved.includes(b.id))-Number(saved.includes(a.id)));content.querySelector('#candidates').innerHTML=candidates.map(f=>`<button class="candidate" data-target="${f.id}" data-index="${index}">${esc(f.name)}${saved.includes(f.id)?' · Guardada':''}<small>${esc(groupNames[f.group]||f.group)}</small></button>`).join('')||'<p class="empty">No está en el catálogo verificado. No calcularemos una porción inventada.</p>';};search.addEventListener('input',update);update();search.focus();
}
function proposal(index,target){
 ...
 content.innerHTML=`<div class="dialog-top"><h2 id="dialog-title">Revisa el cambio</h2><button data-back>Volver</button></div><div class="compare"><div><small>Original</small><strong>${esc(foods[original.foodId].name)}</strong>${esc(qty(original))}</div><div><small>Tu elección</small><strong>${esc(foods[target].name)}</strong>${valid?esc(qty(result)):'Cantidad pendiente de confirmar'}</div></div><div class="${valid?'success':'warning'}">${valid?'Conserva los equivalentes del mismo grupo: '+esc(groupNames[foods[target].group])+'.':esc(result.reason)}</div><p class="source">Tabla: páginas ${foods[original.foodId].page||'sin dato'} y ${foods[target].page}. ${valid?'Solo se cambia este ingrediente; no se añade aceite ni se modifican acompañamientos.':'No se aplicará al menú ni a compras hasta que exista una equivalencia verificada.'}</p><div class="actions"><button class="primary" data-confirm="${target}" data-index="${index}">${valid?'Confirmar solo para este día':'Guardar solicitud por confirmar'}</button>${valid?`<button data-alternative="${target}" data-index="${index}">Guardar como alternativa</button>`:''}</div>`;
}
function confirmSwap(index,target){
 ...
 if(result.status==='valid'&&!current.blockSwaps){const d=store.data.changes[day]??={};(d[current.id]??={})[index]=target;toast('Cambio confirmado solo para este día.');}
 else{const d=store.data.pending[day]??={};d[current.id+':'+index]={recipeId:current.id,index,target,reason:current.warning||result.reason};toast('Solicitud guardada. El menú sigue igual.');}
